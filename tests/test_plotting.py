...
def test_plot_2d_hist():
    ...
    # this creates a png
    test_hist_arrays = plot_data.plot_2d_hist(
        test_dataframe,
        test_title,
        test_xlabel,
        test_ylabel,
...
def test_plot_deaths_imd_decile():
    ...
    output_name = plot_data.plot_deaths_imd_decile(
        test_df, test_title, test_xaxis_label, test_yaxis_label, write_file=False
    )
    expected_output_name = (
        "covidrover/static/png/" + test_title.title().replace(" ", "") + ".png"
    )
...
def test_plot_deaths_imd_decile_writefile():
    ...
    _output_name = plot_data.plot_deaths_imd_decile(
        test_df, test_title, test_xaxis_label, test_yaxis_label, write_file=True
    )
    expected_weboutput_name = (
        "covidrover/static/png/" + test_title.title().replace(" ", "") + ".png"
    )
...
def test_geojson_generation():
    ...
    assert (
        str(type(test_geosource)) == "<class 'bokeh.models.sources.GeoJSONDataSource'>"
    )
